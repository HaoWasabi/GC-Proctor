...
def _resolve_workspace_path(path_str: str) -> Path:
    ...
    if normalized.startswith("src/"):
        ...
        candidates.extend([Path(trimmed), base_dir / trimmed, project_root / trimmed])
...
def _render_mindmap_inline(nodes: list, viewer_file: str) -> None:
    viewer_path = _resolve_workspace_path(viewer_file or "tmp/gc_mindmap.html")
...
def _show_inline_mindmap_panel(active_session_id: str = "") -> None:
    ...
    viewer = _safe_str(payload.get("viewer")) or "tmp/gc_mindmap.html"
...
def generate_flashcard_js_file(cards: list, output_file: str = "tmp/flashcard_data.js") -> None:
...
def _show_inline_flashcard_panel(active_session_id: str = "") -> None:
    ...
    viewer_file = payload.get("viewer", "tmp/gc_flashcard.html")
    data_file = payload.get("dataFile", "tmp/flashcard_data.js")
...
def _generate_unified_chat_answer(mode: str, prompt: str, student_id: str):
    ...
    if _is_mindmap_request(prompt):
        mindmap_result = generate_mindmap_payload(study_svc, prompt, context, chunks, output_file="tmp/mindmap_data.js")
    ...
    for idx, msg in enumerate(messages):
        ...
        with st.chat_message(role):
            ...
            if isinstance(mindmap_nodes, list) and mindmap_nodes:
                ...
                viewer_file = _safe_str(citations.get("viewer")) or "tmp/gc_mindmap.html"
                data_file = _safe_str(citations.get("dataFile")) or "tmp/data.js"
            ...
            if isinstance(flashcard_cards, list) and flashcard_cards:
                fc_viewer_file = "tmp/gc_flashcard.html"
                fc_data_file = "tmp/flashcard_data.js"
                ...
                if isinstance(entities, dict) and entities.get("type") == "mindmap" and isinstance(entities.get("nodes"), list):
                    if st.button("🗺️ Xem sơ đồ", key=f"legacy_mindmap_btn_{idx}"):
                        try:
                            ...
                            generate_mindmap_js(
                                json.dumps({"nodes": mindmap_nodes}, ensure_ascii=False),
                                output_file="tmp/data.js",
                            )
                            st.session_state.inline_mindmap_payload = {
                                "nodes": mindmap_nodes,
                                "viewer": "tmp/gc_mindmap.html",
                            }
                            st.success("✅ Đã cập nhật dữ liệu tại tmp/data.js.")
                            st.rerun()
                        except Exception as e:
                            st.error(f"❌ Không thể render sơ đồ: {e}")
                ...
                with st.spinner("Đang lục tìm trong toàn bộ hệ thống..."):
                    ...
                    try:
                        ...
                        # --- NHÁNH 3: VẼ SƠ ĐỒ ---
                            ...
                            mindmap_result = generate_mindmap_payload(study_svc, prompt, context, chunks, output_file="tmp/data.js")
